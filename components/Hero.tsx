...
const Hero: React.FC = () => {
  ...
  const categories = [
    { name: 'Tech & Coding', icon: <Monitor size={24} className="w-4 h-4 md:w-6 md:h-6" />, color: 'text-red-500', bg: 'bg-red-50' },
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center items-center pt-24 md:pt-32 pb-10 md:pb-20 px-4 overflow-hidden bg-gray-900">
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center w-full">
        ...
        <div className="animate-fade-in-up [animation-delay:300ms] mb-6 md:mb-8">
           <span className="inline-block bg-brand-600 text-white text-[10px] md:text-sm font-bold px-3 py-1 rounded-full animate-pulse">
        ...
        {/* Categories Grid */}
        <div className="animate-fade-in-up [animation-delay:600ms] grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-6 max-w-4xl mx-auto">
          {categories.map((cat, index) => (
            <button 
              key={index}
              onClick={() => handleCategoryClick(cat.name)}
              className="group bg-white rounded-lg md:rounded-xl p-3 md:p-8 flex flex-row md:flex-col items-center justify-start md:justify-center gap-3 md:gap-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-900/20 active:scale-95"
            >
              ...
              <span className="font-bold text-gray-900 text-[10px] md:text-base text-left md:text-center">{cat.name}</span>
            </button>
          ))}
        </div>
      ...
      <div className="absolute bottom-0 left-0 w-full h-12 md:h-24 bg-gradient-to-t from-white to-transparent z-0"></div>
    </section>
  );
};
